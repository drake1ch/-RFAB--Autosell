...
# RFAB Autosell (prototype)

- While the `BarterMenu` is open, press `M` to sell all player inventory items that are **not** favorited.
- Safety: skips favorited, equipped/worn, quest items, keys, lockpicks, and gold.
- Note: price is based on the item's value (`InventoryEntryData::GetValue()`), not the full vanilla barter formula.
